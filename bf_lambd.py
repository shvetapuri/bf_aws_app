...
def set_feedInfo(intent, session):
	...
	if 'whichBreast' in intent['slots']:
		...
		if 'howLong' in intent['slots']:
			howLongFed = intent['slots']['howLong']['value']
			...
			reprompt_text = "Please say right or left to indicate " \
	                        "which breast you last fed from and say the number of min fed"
		else: 
...
def get_feedInfo(intent, session):
	...
	item=response['Item']
	whichBreast=str(item['whichBreast'])
	howLong = str(item['howLong'])
	print("breast is")
	print(whichBreast)

	if whichBreast and howLong:
		speech_output = "You last fed from the " + whichBreast + \
	                        "breast for"+ howLong +"minutes."
		should_end_session = True
	else:
...
def stopTiming(intent, session):
	...
	if startTime:
		...
		print("duration" + minutesLasting)
		
		table.update_item (
			Key= {
				'nameId':nameId,
			},
			UpdateExpression='SET howLong = :val1',
			ExpressionAttributeValues={
				':val1': minutesLasting
			}
		)
		speech_output = "Breastfeeding timer stopped, you fed from the " + whichBreast + " for " \
